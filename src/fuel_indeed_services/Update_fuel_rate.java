...
/**
 * Servlet implementation class Update_fuel_rate
 */
public class Update_fuel_rate extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			System.out.println(fdieselrate);
			
			Connection con=ConnectDB.dbcon();
			
			PreparedStatement ps=con.prepareStatement("update fuelstation set fpetrolrate=?, fdieselrate=? where femail=?");
			ps.setInt(1,Integer.parseInt(fpetrolrate));
			ps.setInt(2,Integer.parseInt(fdieselrate));
